...
class AsistenteReporteCompras(models.TransientModel):
    ...
    def print_report_excel(self):
        for w in self:
            ...
            hoja.write(y, 14, 'Diario de Pago')

            #accounts
            account_type_data = self.env['account.account.type'].search([])
            journal_ids = [x for x in dict['diarios_id']]
            invoice_data = self.env['account.invoice'].search([
                ('type', '=', 'in_invoice'),
                ('journal_id', 'in', journal_ids),
                ('state', 'in', ['open', 'paid']),
                ('date', '>=', dict['fecha_desde']),
                ('date', '<=', dict['fecha_hasta']),])
            
            id_accounts = []
            for invo in invoice_data:
                for line_invo in invo.invoice_line_ids:
                    id_accounts.append(line_invo.account_id.id)
            
            accounts_ids = self.env['account.account'].search([('id', 'in', id_accounts)], order='id asc')
            
            col_acc = 16
            account_y = y
            col_acc_data = 16
            for type_accou in account_type_data:
                for account_col in accounts_ids:
                    if account_col.user_type_id.id == type_accou.id:
                        col_acc += 1
                        hoja.write(y, col_acc, account_col.name)
                    ...
                    for account_col in accounts_ids:
                        if account_col.user_type_id.id == type_accou.id:
                            ...
                            for invoice in invoice_data:    
                                if str(line['id']) == str(invoice.id):
                                    for invoice_accounts in invoice.invoice_line_ids:
                                        if invoice_accounts.account_id.id == account_col.id:
                                            account_amount += invoice_accounts.price_subtotal
                            col_acc_data += 1
                            hoja.write(account_y, col_acc_data, account_amount)
