...
from odoo.addons.web_service_integration.models import invoice_special
from odoo.addons.web_service_integration.models import nota_abono
...
class AccountInvoice(models.Model):
    ...
    def action_invoice_open(self):
        ...
        if self.type == "out_invoice":
            if not self.numero_dte:
                ...
                message = _("Facturación Electrónica: Serie %s  Número %s") % (serie, numero_dte)
                self.message_post(body=message)
                self.uuid = uuid
                self.serie = serie
                self.numero_dte = numero_dte
                url_invoice_fel = 'https://report.feel.com.gt/ingfacereport/ingfacereport_documento?uuid=' +str(uuid)
                exist_url = self.env['account.invoice'].sudo().search([('url_invoice_fel','=',url_invoice_fel)])
                if exist_url:
                    raise UserError('El número de factura Fel ya existe en Odoo, valide nuevamente su factura o actualice su navegador')
                self.url_invoice_fel = 'https://report.feel.com.gt/ingfacereport/ingfacereport_documento?uuid=' +str(uuid)
        ...
        if self.type == "out_refund" and self.refund_invoice_id:
            if not self.numero_dte:
                ...
                message = _("Nota de Crédito: Serie %s  Número %s") % (serie, numero_dte)
                self.message_post(body=message)
                self.uuid = uuid
                self.serie = serie
                self.numero_dte = numero_dte
    ...
    def action_invoice_cancel(self):
        ...
        if self.type == "out_invoice":
           ...
           message = _("Factura Cancelada: Serie %s  Número %s") % (serie, numero_dte)
           self.message_post(body=message)
           self.uuid = uuid
           self.serie = serie
           self.numero_dte = numero_dte
        ...
        if self.type == "out_refund" and self.uuid:
           xml_data = invoice_cancel.set_data_for_invoice_cancel(self)
           uuid, serie, numero_dte, dte_fecha =invoice_cancel.send_data_api_cancel(self, xml_data)
           message = _("Nota de Crédito Cancelada: Serie %s  Número %s") % (serie, numero_dte)
           self.message_post(body=message)
           self.uuid = uuid
           self.serie = serie
           self.numero_dte = numero_dte
